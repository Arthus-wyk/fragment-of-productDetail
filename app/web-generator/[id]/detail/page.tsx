...
export default function WebGeneratorDetail() {
    ...
    onFinish: async ({ object: fragment, error }) => {
      if (!error) {
        if (fragment) {
          ...
          updateCode(supabase, chat_id, fragment.code, 'detail')
        }
      }
    },
  })
  ...
  const handleAISubmit = (value: any) => {
    if (result) {
      const content: Message['content'] = [
        { type: 'text', text: formPrompt(result.code, JSON.stringify(value)) },
      ]
      submit({
  ...
  return (
    ...
    >
      <SheetContent side="left" className="w-[600px] sm:w-[740px] overflow-visible z-50">
        <div className="p-4">
          <DetailForm handleAISubmit={handleAISubmit} />
        </div>
      </SheetContent>
